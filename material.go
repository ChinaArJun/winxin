...
const (
	...
	MaterialUploadNews         = "https://api.weixin.qq.com/cgi-bin/media/uploadnews?access_token=%s"
	MaterialPreviewNews        = "https://api.weixin.qq.com/cgi-bin/message/mass/preview?access_token=%s"
)
...
// UploadMaterialPreviewNews 预览接口【订阅号与服务号认证后均可用】
func UploadMaterialPreviewNews(news PreviewNewsMsg) (err error) {
	url := fmt.Sprintf(MaterialPreviewNews, AccessToken())
	wapper := &struct {
		WXError
		MsgID int `json:"msg_id"`
	}{}
	err = Post(url, news, wapper)
	return err
}

// UploadMaterialUploadnews 上传图文消息素材
func UploadMaterialUploadnews(news MaterialNews) (mediaId string, createAt int, err error) {
	url := fmt.Sprintf(MaterialUploadNews, AccessToken())
	wapper := &struct {
		WXError
